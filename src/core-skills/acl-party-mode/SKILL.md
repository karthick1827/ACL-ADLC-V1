---
name: acl-party-mode
description: 'Orchestrates lively group discussions between installed ACL agents or custom personas, and helps author custom parties. Use when the user requests party mode, a roundtable, or multiple agent perspectives — or wants to create/configure a party, define personas, or build an AI focus-group panel.'
---
...
- **Scripts** (run via `uv run`): `{project-root}/_acl/scripts/resolve_config.py` resolves central config (four-layer TOML merge); `{project-root}/_acl/scripts/resolve_customization.py` resolves `{workflow.*}`; `{skill-root}/scripts/resolve_party.py` resolves the roster, `party_mode`, `memory_enabled`, and scene/`open_cast`; `{project-root}/_acl/scripts/memlog.py` reads/writes per-party memory.
...
2. **Resolve core config:** `uv run {project-root}/_acl/scripts/resolve_config.py --project-root {project-root}`. From the merged JSON's `core` table: greet with `{user_name}`, speak in `{communication_language}`, and resolve `{output_folder}`; `{date}` is today's date.
