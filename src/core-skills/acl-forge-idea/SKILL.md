...
- Scripts live in two places — run each from the exact path written, never assume co-location: the shared core scripts (`memlog.py`, `resolve_customization.py`, `resolve_config.py`) are installed by ACL core at `{project-root}/_acl/scripts/` and are never bundled here; this skill's own `resolve_personas.py` is at `{skill-root}/scripts/`.
- `{workflow.<name>}` resolves to fields in the merged `customize.toml` `[workflow]` table.

## On Activation

1. Resolve customization: `uv run {project-root}/_acl/scripts/resolve_customization.py --skill {skill-root} --key workflow`. On failure, read `{skill-root}/customize.toml` directly with defaults. Apply the resolved `{workflow.*}` values throughout.
...
3. Resolve central config: `uv run {project-root}/_acl/scripts/resolve_config.py --project-root {project-root} --key core`; from the merged JSON read `{user_name}`, `{communication_language}`, `{output_folder}`. On failure use neutral defaults; never block. Greet `{user_name}` in `{communication_language}` and stay in it.
4. Note whether a ACL persona is already active in this conversation — the user loaded one (e.g. the analyst, the storyteller) and invoked the forge from within it. If so, that persona leads the session, in voice, throughout.
...
`uv run {project-root}/_acl/scripts/memlog.py init --workspace {workspace} --field idea="<idea>" --field goal="<goal>"`
...
`uv run {project-root}/_acl/scripts/memlog.py append --workspace {workspace} --type <decision|assumption|crack|kill|direction|lock|note> --text "<gist>"`
...
If a ACL persona was already active when the forge started, keep that persona as the lead voice.
...
The script returns installed ACL agents (`agents`), user-defined personas (`members`), and saved parties (`parties`). Parties may include a `scene`; some are open-cast. This gives you the same roster information as `acl-party-mode` without invoking it.
...
Flip the status at the end: `uv run {project-root}/_acl/scripts/memlog.py set --workspace {workspace} --key status --value complete`.
