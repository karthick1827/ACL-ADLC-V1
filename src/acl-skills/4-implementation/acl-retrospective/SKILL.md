...
- `agent_roster` = resolved via `python3 {project-root}/_acl/scripts/resolve_config.py --project-root {project-root} --key agents` (merges four layers in order: `_acl/config.toml`, `_acl/config.user.toml`, `_acl/custom/config.toml`, `_acl/custom/config.user.toml`)
