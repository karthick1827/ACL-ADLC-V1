...
At the opening greeting, let the user know they can invoke `acl-party-mode` for multi-agent perspectives or `acl-advanced-elicitation` for deeper exploration at any point.
...
5. Load `{project-root}/_acl/acl/config.yaml` (and `config.user.yaml` if present). Resolve `{user_name}`, `{communication_language}`, `{document_output_language}`, `{planning_artifacts}`, `{project_name}`, `{date}`.
