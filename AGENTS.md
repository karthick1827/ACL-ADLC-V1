...
- **Universal Sequential Document Gate for ALL Agents & Skills**:
  - EVERY single agent (Sally UX Designer, Winston Architect, Amelia Developer, Mary Analyst, etc.) and EVERY skill (`acl-architecture`, `acl-ux`, `acl-create-epics-and-stories`, `acl-quick-dev`, `acl-figma-bridge`, etc.) MUST FIRST verify that ALL upstream documents in `_acl-output/` have `status: Accepted`.
  - If **ANY** prerequisite document in `_acl-output/` is missing, or has `status: In Review` or `status: Rejected`:
...
## 🛑 STRICT PROHIBITION: No Direct AI Status Manipulation & No Self-Approval Prompts
- The AI agent is **STRICTLY PROHIBITED** from using tools (`replace_file_content`, `write_to_file`, `run_command`, etc.) to change `status: In Review` -> `status: Accepted`.
